...
def getMeals(request):
    try:
        ...
        system_message = (
                ...
                'Send the response in the following JSON format { meals: [ {name: "meal-item-1", quantity: "300 grams", imageUrl: "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcSK6ghC4OB1yk7uqJoEupPCdoFFfYecE5J_UQ&s" , benefits: "This meal contains good number of fats and anti-oxidants and many other health benifits" }, {name: meal-item-2, quantity: 500 grams, imageUrl:"https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcSK6ghC4OB1yk7uqJoEupPCdoFFfYecE5J_UQ&s",  benifits: "This meal contains good number of fats and anti-oxidants and many other health benifits" } ] }.'
                'please use an active imageUrl else go with the given url. please do not generate any extra characters other than the json. The response should be a proper JSON string'
            )
...
def getWorkout(request):
    try:
        ...
        system_message = (
                ...
                'Send the response in the following JSON format { [ {name: "Exercise-1(example: push-ups)", sets: "number of sets(example:5)", reps: "number of repetitions (example:20)", benefits: "this exercise is good for your arms and shoulders which increases strength and stabitlity" }, {name: "Exercise-2(example: pull-ups)", sets: "number of sets(example:5)", reps: "number of repetitions (example:20)", benifits: "this exercise is good for your arms and shoulders which increases strength and stabitlity" } ] }.'
                'Avoid any unwanted nesting like { excercises: [ { name: "pushups" } ] }, and please do not generate any extra characters other than the json. The response should be a proper JSON string'
            )
        response = model.generate_content(system_message)
        generated_text = clean_json_string(response.text)
        # data = json.loads(generated_text)    
